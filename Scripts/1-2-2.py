...
def WKNN( test_point ,k):   #file为文件名：xxxx.xlsx
    list_last = []
    list_data_test = get_data( r'C:\Users\Administrator\Desktop\RSS' + '\\' + test_point )
    ...
    denominator = 0
    list_index = 0
    list_min_copy = list_min.copy()
    list_min_copy.sort()
    for i in list_min:
        if i==0:
            print('出现0')
            print(list_min_copy)
            for ind in list_min_copy:
                if ind!=0:
                    i = (1/2)*ind
                    break
    ...
    for i in range(k):
        ...
        if list_min[i]==0:
            for ind in list_min_copy:
                if ind != 0:
                    weighted.append((1/ind)/ denominator)
                    break
                list_index = list_index + 1
        else:
